...
def call_blast(query, out_dir, outfmt, db='nucl', blast_type="n"):
    ...
    if str.lower(blast_type) in ("p", "blastp", "protein"):
        cmd = f"blastp -query {query} -db {db} -outfmt '{outfmt}' -out {out_file} -max_target_seqs 20 -remote"
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-fmt", "--outfmt", help="Blast format type.", default="7", type=str)
    ...
    for query in files:
        try:
            call_blast(query, args.out_dir, args.outfmt, args.ncbi_db, args.blast_type)
        except:
            raise
            pass
